...
### §6cr — MOMENT-POLE SUB-LAW: the mechanism behind §6cq (probe_qmin_moment_pole_sublaw.py, L9)
  ...
  MECHANISM (COMPLETE STRUCTURAL PROOF, order-2 recurrence + explicit base-case induction — supersedes both the
  earlier β-quartet guess AND the "modulo routine residue" degree-drop sketch).  The B-matrix (Chebyshev)
  collapses the moment generating function: the moments satisfy a minimal ORDER-2 recurrence
  ...
  (both verified over 364 Row-3 orbits n<70), so c₁ = B/N, c₂ = −C/N.  For p|N with v_p(N)=1, fix a Gaussian
  prime 𝔭|p.  THREE verified facts (1060 simple-factor checks, n<70):
        (i)  v_p(B)=0   ⟹ v_𝔭(c₁) = v_p(B)−v_p(N) = −1;
        (ii) v_p(C)≥0   ⟹ v_𝔭(c₂) = v_p(C)−v_p(N) ≥ −1  (automatic: C∈ℤ);
        (iii) BASE CASE  v_p(w₀)=−1,  v_p(w₁)=−2.
  INDUCTION (no cancellation).  If v_p(w_{i−1})=−i and v_p(w_{i−2})=−(i−1) then
        v_𝔭(c₁w_{i−1}) = −1 + (−i)         = −(i+1),
        v_𝔭(c₂w_{i−2}) = (≥−1) + (−(i−1))  ≥ −i  >  −(i+1).
  The c₁-term is STRICTLY more negative ⇒ no cancellation ⇒ v_p(w_i) = −(i+1) EXACTLY.  QED (no residue
  computation needed).  ⇒ v_p(w_{m−1}) = −m.
