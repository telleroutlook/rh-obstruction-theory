...
or a Row-3-specific powerful-gap argument for gaps of size $\asymp A$).
...
\item $\Aplus\equiv\Aminus\pmod{8}$; the common value is $5\pmod{8}$
  if $4\nmid n$, and $1\pmod{8}$ if $4\mid n$.
\end{enumerate}
...
%---------------------------------------------------------------------
\section{Theorem E: square-based powerful obstruction (NT-C $t=1$, complete)}
\label{sec:thmE}% xref-only
%---------------------------------------------------------------------

\begin{mainthm}[NT-C $t=1$: complete obstruction]
\label{thm:squarepow}
For every Row-3 pair $(a,n)$, $\Aplus$ and $\Aminus$ cannot simultaneously be of
the form $5^e m^2$ (square-based powerful-away-from-$5$).
\begin{enumerate}[label=\textup{(\roman*)}]
\item \textup{(Complete, $4\mid n$).}  Neither $\Aplus$ nor $\Aminus$ individually
  equals $5^e m^2$ for any odd $e\geq 1$; the simultaneous perfect-square case
  ($e$ even for both) is excluded by Theorem~\ref{thm:square}.
\item \textup{(Complete, $4\nmid n$).}  Even $e$ is impossible individually
  ($5^{e_{\mathrm{even}}}m^2\equiv\{0,1,4\}\pmod8$ while $\Aplus\equiv5\pmod8$).
  The simultaneous case $(\Aplus,\Aminus)=(5S^2,5T^2)$ is excluded by a direct
  pair of 5-conics reducing to the elliptic curve
  $E\colon Y^2=X^3-32X+64$, which has rank~$0$ and torsion~$\mathbb{Z}/4\mathbb{Z}$;
  all rational torsion points pull back to $a=n/2$, forcing $n=2$.
\end{enumerate}
\end{mainthm}

\begin{proof}
\emph{Part (i)} is proved as before: for $4\mid n$, odd $e$ gives $5T^2\in\{0,4,5\}
\pmod{8}$ while $\Aplus\equiv1\pmod{8}$, a contradiction; even $e$ is
Theorem~\ref{thm:square}.

\emph{Part (ii), even $e$:} $5^e m^2=\square\in\{0,1,4\}\pmod{8}$, contradicting
$\Aplus\equiv5\pmod{8}$.

\emph{Part (ii), odd $e$ — corrected 5-conic reduction.}
Put $b=n-a$, $x=b/n$, $r=T/n$, and $s=S/n$.  Dividing by $n^2$ gives
$$x^2+1=5r^2,\qquad x^2-2x+2=5s^2.$$
Parametrising the first conic from $(x,r)=(2,1)$ gives
$$x=\frac{2(5t^2-5t+1)}{5t^2-1},\qquad
  r=-\frac{5t^2-4t+1}{5t^2-1}.$$
Writing $Y=(5t^2-1)s$, the second conic becomes
$$Y^2=10t^4-20t^3+24t^2-12t+2.$$
Set $u=t-1$ and $v=Y$.  For $u\ne0$, define
\[
  \begin{aligned}
    A&=v+2+4u+2u^2,\quad &B&=v(1+u)+2+6u+6u^2,\\
    X&=\frac{4A}{u^2},\quad &Y_E&=\frac{16B}{u^3}+40.
  \end{aligned}
\]
Then
\[
  Y_E^2-(X^3-32X+64)=
  \frac{64A(10u^4+20u^3+24u^2+16u+4-v^2)}{u^6},
\]
so this maps the corrected quartic to $E$.  Away from the four torsion points,
the inverse is
\[
  u=\frac{-4X}{4X-Y_E+8},\qquad
  v=\frac{2(2X^3-Y_E^2+16Y_E-64)}{(4X-Y_E+8)^2},\qquad t=1+u.
\]
The cleared quartic error is
$16X^3(Y_E^2-X^3+32X-64)/(4X-Y_E+8)^4$, hence the inverse maps $E$ to the
quartic.

PARI/GP returns $\mathrm{rank}(E(\mathbb{Q}))=0$ and
$E(\mathbb{Q})_{\mathrm{tors}}=\mathbb{Z}/4\mathbb{Z}
=\{\mathcal{O},(4,0),(0,\pm8)\}$.  Their pullbacks are respectively
$(t,Y)=(1,2),(1/3,2/9),(1,-2),(1/3,-2/9)$.  Each gives $x=1/2$, hence
$a=n/2$; the coprimality condition forces $n=2$, contradicting $n\ge4$.
No case survives; the proof is complete.\qed
\end{proof}

\begin{remark}
No assumption on RH or Sha($E$) is imported.  PARI/GP~2.17.4 independently
returns certified rank bounds $[0,0]$ on $E$, its translated 2-torsion model,
its 2-isogenous model, and the corrected quartic Jacobian; the replay is recorded in
\texttt{verify\_OE02\_pari\_replay.py}.  The torsion, Nagell--Lutz, and finite
quartic scans are verified in \texttt{verify\_OE02\_elliptic.py}.  The conic
substitution and both birational-map identities are verified in
\texttt{verify\_OE02\_quartic\_map.py}.
The corrected 5-conic reduction and torsion pullback remain subject to independent human
review, so the mathematical status remains PROOF-DRAFT.
\end{remark}
...
An S-unit approach over $\Zi$ must retain the odd-exponent cube factors.
In full, write
\[
w_+=\varepsilon_+(1+2i)^{f_+}(1-2i)^{g_+}U_+\xi_+^2,\qquad
w_-=\varepsilon_-(1+2i)^{f_-}(1-2i)^{g_-}U_-\xi_-^2,
\]
where $U_\pm$ records Gaussian primes occurring to an odd exponent at least
three.  The shift identity $w_+-w_-=2a-n$ then gives
\[
\varepsilon_+V_+U_+\xi_+^2-\varepsilon_-V_-U_-\xi_-^2=2a-n
\]
with $V_\pm$ the displayed $5$-parts.  Since $\gcd(a,n)=1$ implies
$\gcd(A^\pm,n)=1$, the non-5 support of $U_\pm\xi_\pm$ is not contained in
$S(n)$; it is supplied by the factor sets of $A^\pm$ themselves.  Thus
$S_0\cup S(n)$ is not the actual support, and the support grows through new
split primes in $A^\pm$, not merely through $\omega(n)$.

\medskip
\noindent\emph{What is needed} \textup{(NT-A)}: a uniform effective theorem for the
above varying-support equation, with explicit heights in the Row-3 data and the
actual supports of $U_\pm\xi_\pm$.  The Evertse--Schlickewei--Schmidt theorem
\cite{Evertse1984,Schmidt1990} and its Gaussian analogues give finiteness only
after an actual finite support set (and normalized equation) is fixed; they
give neither a cardinality-only height bound nor a uniform theorem for these
moving supports.
\end{theorem}
...
\medskip
\noindent\emph{What is needed} \textup{(NT-D)}: a Row-3-specific
powerful-gap bound for gaps $k\asymp A$, an $abc$ conjecture over $\Zi$
with an exponent strictly below $3$ in $\mathrm{rad}(\Aplus\Aminus k)$,
or an independent Diophantine argument. This cannot be a gap-only statement:
$4=2^2$ and $8=2^3$ are powerful away from $5$ and differ by a quantity comparable
to their size. The needed input must retain the Row-3 coupling (common $n$,
coprimality, congruences, and Gaussian norm shape).
\end{theorem}
...
\midrule
NT-A & Varying-support effective S-unit theorem & S-unit equations \\
...
& \emph{Theorem~\ref{thm:squarepow}: $t=1$, PROOF-DRAFT.} & \\
NT-D & Row-3-specific powerful-gap bound for $k\asymp A$ & Gap argument \\
\bottomrule
\end{tabular}
\end{table}
...
\item \textbf{NT-C (Theorem~\ref{thm:NTC}) cube-factor case is the remaining open entry point.}
  Theorem~\ref{thm:square} resolves simultaneous perfect squares via 2-isogeny
  descent on the elliptic curve $E: Y^2=X(X+4)(X-1)$ (rank~0;
  Corollary~\ref{cor:NTC-square}).  Theorem~\ref{thm:squarepow} extends this
  to the square-based $t=1$ forms $5^e m^2$ for both $4\mid n$ and
  $4\nmid n$; its rank computation has an independent PARI/GP replay recorded
  above.
...
\item \textbf{Genus-1 family for the square-only sub-case.}
  ...
  $\mathbb{P}^3$ parametrized by $n$).  Effective $2$-descent could close a
  rank-zero family; Chabauty applies only to rank one with an explicit
  Mordell--Weil setup, and rank at least two requires another effective bound.
