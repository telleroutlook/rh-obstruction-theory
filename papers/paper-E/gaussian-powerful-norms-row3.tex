...
\item \textup{(Complete, $4\nmid n$).}  Even $e$ is impossible individually
  ...
  The simultaneous case $(\Aplus,\Aminus)=(5S^2,5T^2)$ is excluded by a
  Gaussian-integer descent reducing to the elliptic curve
  $E\colon Y^2=X^3-32X+64$, which has rank~$0$ and torsion~$\mathbb{Z}/4\mathbb{Z}$;
  all rational torsion points pull back to $n=\pm2$, violating $n\geq4$.
\end{enumerate}
\end{mainthm}
...
\emph{Part (ii), odd $e$ — Gaussian descent.}
Write $z_1=a+ni$ and $z_2=(n{-}a)+ni$ in $\mathbb{Z}[i]$.  Since
$N(z_1)=5S^2$, exactly one of $\pi=1+2i$ or $\bar\pi=1-2i$ divides $z_1$ to odd
valuation; write $z_j=\omega_j\alpha_j^2$ with $\omega_j\in\{u\pi,u\bar\pi\}$
($u$ a unit).  The identity
$$z_1+z_2 = n(1+2i)=n\pi$$
partitions into four cases $(\omega_1,\omega_2)$:
\begin{itemize}
\item \emph{Cases B, C} (one factor $\pi$, one $\bar\pi$): the sum forces the case-$B$
  factor $z_2$ to satisfy $\pi\bar\pi=5\mid z_2$, giving $5\mid(n{-}a)$ and $5\mid n$,
  hence $5\mid a$, contradicting $\gcd(a,n)=1$.
\item \emph{Case D} (both $\bar\pi$): the sum divided by $\bar\pi$ must lie in
  $\mathbb{Z}[i]$, forcing $5\mid n$.  A unit-by-unit analysis of all sub-cases then
  gives $5\mid a$, again contradicting $\gcd(a,n)=1$.
\item \emph{Case A} (both $\pi$): dividing by $\pi$ gives $n$ real.  Mixed or
  doubly-imaginary units produce parity contradictions ($Y_1\equiv0\pmod4$ vs.\
  $X_2$ odd, or $n\equiv0\pmod4$ vs.\ $n\equiv2\pmod4$).  For both-real units,
  setting $x=X_1/Y_1$ reduces the system to
  $$y_1^2 = x^2+1,\quad y_2^2 = x^2+2x+2.$$
  Parametrising the first conic as $x=(1{-}t^2)/(2t)$ yields the quartic
  $Y^2=t^4-4t^3+6t^2+4t+1$.  This is birationally equivalent to the Weierstrass
  model $E\colon Y^2=X^3-32X+64$ (verified: $(4,0),(0,\pm8)$ on $E$;
  $2\cdot(0,8)=(4,0)$, $4\cdot(0,8)=\mathcal{O}$).  A $2$-isogeny descent gives
  $\mathrm{rank}(E(\mathbb{Q}))=0$; the torsion subgroup is
  $\mathbb{Z}/4\mathbb{Z}=\{\mathcal{O},(4,0),(0,\pm8)\}$.  All four torsion points
  pull back to $t\in\{0,\infty\}$, giving $Y_1=0$ and hence $n=\pm2$, violating
  $n\geq4$.
\end{itemize}
...
\begin{remark}
No assumption on RH or Sha($E$) is imported.  The rank-$0$ claim is a
PROOF-DRAFT pending independent CAS verification of the Selmer group computation.
The torsion, Nagell--Lutz, and quartic scans are verified in
\texttt{checker/verify\_OE02\_elliptic.py}.
\end{remark}
...
\item \textbf{NT-C (Theorem~\ref{thm:NTC}) cube-factor case is the remaining open entry point.}
  ...
  Theorem~\ref{thm:squarepow} extends this to all square-based powerful forms
  ($5^e m^2$): complete for $4\mid n$, partial for $4\nmid n$ (the case
  $(\Aplus,\Aminus)=(5S^2,5T^2)$ is numerically absent for $n\leq3000$ but
  analytic proof is outstanding).
