...
| OB-35 | `OB-35-Eprime-refined-degree-resend.md` | E' | **GATE-A (resend)**: abstract odd-meromorphic jet lemma with REFINED leading degree d=3 (exact criterion d=3 ⟺ Δ_1(c)≠0; Δ_1'(0)≠0 computed J=1..4; honest ~10×/J constant degradation); abstract-only, NOT Suzuki | OPEN — send for review |
| OB-34 | `OB-34-H-incomparability-resend.md` | H | **GATE-A (resend)**: incomparability O_finite ⋈ O_theta^{samp} for all K — forward via same-Im quartets, reverse via the B2 exact-collision pair (Gate-A PASS OB-20); scoped to the nonconstant sampled-count map, literal-O_theta comparability caveat retained | OPEN — send for review |
