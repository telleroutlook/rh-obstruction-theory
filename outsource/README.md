...
| OB-30 | `OB-30-Eprime-gate-a-review-package.md` | E' | **GATE-A**: meromorphic (Suzuki-target) compactness | RESOLVED — **GATE-A BLOCKED** (degree z^{2J+3} wrong: J=1 gives z³; (A2) false for real B=iA'; Suzuki entire ⇒ moving-pole incompatible); withdrew Suzuki-target claim, retained abstract lemma; integrated 2026-08-12 |
| OB-31 | `OB-31-Dprime-gate-a-review-package.md` | D' | **GATE-A**: log-polyhomogeneous escape-route audit | RESOLVED — **GATE-A BLOCKED** (Claim A universal false: class omits ellipticity, (I−Δ_x)(I−Δ_y) on 𝕋⁴ has leading log); narrowed to 𝒞_logpoly^{sub,ell}, lemma renamed+PENDING; Claims B/C/D/E confirmed, numerics corrected; integrated 2026-08-12 |
| OB-32 | `OB-32-H-gate-a-review-package.md` | H | **GATE-A**: information-obstruction ordering | RESOLVED — **GATE-A BLOCKED** (same-Im witness refutes O_finite≼O_oracle for ordinates-only oracle; literal O_theta constant ⇒ O_theta≺O_finite, so incomparability NOT established); fixed O_oracle=𝒵, preorder-not-lattice, H'(i) withdrawn; integrated 2026-08-12 |
| OB-35 | `OB-35-Eprime-refined-degree-resend.md` | E' | **GATE-A (resend)**: abstract odd-meromorphic jet lemma with REFINED leading degree d=3 (exact criterion d=3 ⟺ Δ_1(c)≠0; Δ_1'(0)≠0 computed J=1..4; honest ~10×/J constant degradation); abstract-only, NOT Suzuki | RESOLVED — **GATE-A CONDITIONAL → integrated 2026-08-13, M1–M8** (record class redefined ℰ_{k,J}^{mer}(w₀) (i)-(iv); construction symbols (2.1)-(2.5); general Δ₁'(0)≠0 proof via formula (0.1); J=4 anchor corrected to −9.84706645×10⁻⁷; ratios 23.94/32.61/44.04; non-uniformity and q₀ formulations corrected; R_B=+∞ convention; citations to OB-09 §7.1-§7.2 eqs.(7.3)-(7.7) + Suzuki 2013 Thm 2.2(2),(7)); abstract E'-neg lemma PROOF-DRAFT → INDEPENDENTLY-CHECKED |
| OB-34 | `OB-34-H-incomparability-resend.md` | H | **GATE-A (resend)**: incomparability O_finite ⋈ O_theta^{samp} for all K — forward via same-Im quartets, reverse via the B2 exact-collision pair (Gate-A PASS OB-20); scoped to the nonconstant sampled-count map, literal-O_theta comparability caveat retained | RESOLVED — GATE-A CONDITIONAL → integrated 2026-08-13, M1–M8 |
| OB-33 | `OB-33-Dprime-subell-lemma-gate-a.md` | D' | **GATE-A (resend)**: narrowed 𝒞_logpoly^{sub,ell} leading-singularity lemma (elliptic principal symbol + strictly-lower-order log ⇒ pure-power leading heat trace; Hörmander 1968 Thm 1.1 + Karamata; (I−Δx)(I−Δy) shows ellipticity load-bearing) | RESOLVED — **GATE-A CONDITIONAL → integrated 2026-08-13** (M1–M9: scalar class def + K∈ℕ₀ + remainder estimates; comparison/min–max proof replacing Seeley-direct; deleted O(Λ^{(d-1)/m}); shifted Karamata; {γ_n} defined (all nontrivial zeros, no β=1/2); Hörmander Thm 1.1→4.4+p.215; Titchmarsh–Heath-Brown Thm 9.4 added; T⁴ axes→coordinate-subspaces; ellipticity sufficient not iff; 7 numerical anchors deleted); `LEADING-SINGULARITY-COVERS-SUBPRINCIPAL-LOGPOLY` PENDING → INDEPENDENTLY-CHECKED |
